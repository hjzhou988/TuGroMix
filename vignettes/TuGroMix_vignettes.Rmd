...
## Test by GAMM
use gamFit to test growth rate difference between groups.
```{r}
gamfit.res = gamFit(tv.data = tv_wide.long,ref.group = "G1",group.id.var = "Group",mouse.id.var = "Animal.No.")

knitr::kable(gamfit.res$coefs)
```

## Visualize data points and fitted smooth curves
Plot gamFit result
```{r}
plot(gamfit.res)
```
...
screen.res = lmmscreen(tv.data = tv.data.screen, ref.group = "Vehicle",model.var = Model, group.var = Trt, mouse.id.var = Mouse.ID,
...
## Screen for biomarkers that affect drug efficacy by GAMM
```{r}
## Use GAMM instead of LMM for biomarker screening. It may take a long time (10 to 20 times more time than LMM)
gam.screen.res = gamscreen(tv.data = tv.data.screen, ref.group = "Vehicle",model.var = Model, group.var = Trt, mouse.id.var = Mouse.ID,
          gene.expr.data = expre.screen.X)

## screening result is ordered by p-values.
knitr::kable(gam.screen.res)
```
